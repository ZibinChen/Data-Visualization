...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        sample_selected = np.random.randint(len(XX), size = n)
        X = [item[0:-1] for item in XX]
        y = [item[-1] for item in XX]
        samples = [] # sampled dataset
        labels = []  # class labels for the sampled records
        
        for i in sample_selected:
            samples.append(X[i])
            labels.append(y[i])
    ...
    def bootstrapping(self, XX):
        ...
        n = int(np.ceil(2/3 * len(XX)))
        for i in range(self.num_trees):
            data_sample, data_label = self._bootstrapping(XX, n)
